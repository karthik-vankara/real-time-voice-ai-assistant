        ...
        for (const event of events) {
            if (event.event_type === 'tts_audio_chunk') {
                ...
                if (!session.isComplete && is_last) {
                    console.log(`[TTS FINAL CHUNK] Got is_last flag at chunk ${chunk_index}. Stored so far: ${session.chunks.size}`);
                ...
                if (session.gotFinalFlag && !session.isComplete) {
                    ...
                    if (missingChunks.length === 0) {
                        ...
                        console.log(`[TTS READY] All ${actualCount} chunks received (0-${session.expectedFinalIndex}). Starting decode...`);
                        ...
                        const decodeAudio = async () => {
                            try {
                                ...
                                console.log(`[TTS DECODED] Total audio data: ${allBytes.length} bytes, from ${session.chunks.size} chunks`);
                        ...
                        console.log(`⏳ Waiting for chunks... Have ${actualCount}/${expectedCount}. Missing: [${missingChunks.join(', ')}]`);
                    }
                }
            }
        }
